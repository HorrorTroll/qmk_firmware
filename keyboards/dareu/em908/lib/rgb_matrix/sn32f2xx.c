...
static uint16_t       led_state[SN32F2XX_LED_COUNT];     // led state buffer
...
void sn32f2xx_set_color(int index, uint8_t r, uint8_t g, uint8_t b) {
    ...
    if (flip_gb) {
        chSysLock();
        if (sn32f2xx_load_color(index, 'R') == color_r && sn32f2xx_load_color(index, 'B') == color_g && sn32f2xx_load_color(index, 'G') == color_b) {
            chSysUnlock();
            return;
        }
        sn32f2xx_save_color(index, 'R', color_r);
        sn32f2xx_save_color(index, 'B', color_g);
        sn32f2xx_save_color(index, 'G', color_b);
        chSysUnlock();
        return;
    }
#endif // UNDERGLOW_RBG
    ...
    chSysLock();
    if (sn32f2xx_load_color(index, 'R') == color_r && sn32f2xx_load_color(index, 'B') == color_b && sn32f2xx_load_color(index, 'G') == color_g) {
        chSysUnlock();
        return;
    }
...
void sn32f2xx_set_color_all(uint8_t r, uint8_t g, uint8_t b) {
    ...
    for (int i = 0; i < SN32F2XX_LED_COUNT; i++) {
        ...
        uint8_t color_g = g * SN32F2XX_LED_OUTPUT_LUMINOSITY_G;
        uint8_t color_b = b * SN32F2XX_LED_OUTPUT_LUMINOSITY_B;
        sn32f2xx_save_color(i, 'R', color_r);
        sn32f2xx_save_color(i, 'B', color_b);
...
void sn32f2xx_save_color(uint8_t ledNumber, char colorChannel, uint8_t value) {
    ...
    switch(colorChannel){
        case 'R':
            led_state[ledNumber] = (led_state[ledNumber] & ~0b1111100000000000) | (((value * 31 / 255) << 11) & 0b1111100000000000);
            break;
        case 'G':
            led_state[ledNumber] = (led_state[ledNumber] & ~0b0000011111100000) | (((value * 63 / 255) <<  5) & 0b0000011111100000);
            break;
        case 'B':
            led_state[ledNumber] = (led_state[ledNumber] & ~0b0000000000011111) | (((value * 31 / 255) <<  0) & 0b0000000000011111);
            break;
    }
#elif (SN32F2XX_RGB_MATRIX_COLOR_DEPTH == 8)
    switch(colorChannel){
...
uint8_t sn32f2xx_load_color(uint8_t ledNumber, char colorChannel) {
    ...
    switch(colorChannel){
        case 'R':
            return (uint8_t) ((led_state[ledNumber] & 0b1111100000000000) >> 11) * 8;
        case 'G':
            return (uint8_t) ((led_state[ledNumber] & 0b0000011111100000) >>  5) * 4;
        case 'B':
            return (uint8_t) ((led_state[ledNumber] & 0b0000000000011111) >>  0) * 8;
    }
#elif (SN32F2XX_RGB_MATRIX_COLOR_DEPTH == 8)
    switch(colorChannel){
        case 'R':
